    ...
    const reducedInventors = inventors.reduce((acc, inventor) => acc + (inventor.passed - inventor.year), 0);

    // 5. Sort the inventors by years lived
    const yearsLived = inventors.sort((a, b) => (b.passed - b.year) - (a.passed - a.year));
    // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
    // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
    //const deBoulevards = boulevards.filter(blvd => blvd.includes('de'));
    ...
    const transportation = data.reduce((obj, vType) => {
        if (!obj[vType]) {
            obj[vType] = 0;
        }
        obj[vType]++
